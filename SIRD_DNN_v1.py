...
def print_and_log2train(i_epoch, run_time, tmp_lr, temp_penalty_nt, penalty_wb2s, penalty_wb2i, penalty_wb2r,
    ...
    print('loss for params: %.16f' % loss_r)
...
def solve_SIRD2COVID(params):
    ...
    date, data2S, data2I, data2R, data2D = dataUtils.load_data(filename, N=3450000)
        ...
        for i_epoch in range(params['max_epoch'] + 1):
            t_batch, i_obs = \
                DNN_data.randSample_Normalize_existData(train_date, train_data2i, batchsize=batchSize_train,
                                                        normalFactor=1.0, sampling_opt=params['opt2sample'])
            ...
            _, loss_s, loss_i, loss_r, loss_d, loss, pwb2s, pwb2i, pwb2r, pwb2d = sess.run(
                ...
                feed_dict={T_it: t_batch, I_observe: i_obs, in_learning_rate: tmp_lr,
