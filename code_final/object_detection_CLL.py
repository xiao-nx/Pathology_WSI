...
if __name__ == '__main__':
      ...
      # 数据的输入和输出路径
      data_path_dict = {
            "imageInput": "../dataset/dataRaw/CLL",
            "imageSeg": "../dataset/dataSeg/CLL",
            "imageROI": "../dataset/dataROI/CLL",
            "imageCell": "../dataset/dataCell/CLL"
      }
      ...
      for idx,imageName in enumerate(imageList):
            ...
            #viewImage(imgShow, 'imgShow')

            # 保存处理后的图像
            pattern = re.compile(r'(dataRaw)(.*)(-)')
            imgSegmentationSaveName = pattern.sub(imageSegPagth + '/' + r'CLL-segmetation-', imageName) # 保存图像的路径
            ...
            for c in contours:
                  if cv2.contourArea(c) > 20000:
                        ...
                        imgCellSaveName = pattern.sub(imageCellPath + '/' + r'CLL-cell-'+str(cell_cnt)+'-', imageName)  # 保存图像的路径
                        cv2.imwrite(imgCellSaveName, cell_image)

            # 保存处理后的图像
            imgROISaveName = pattern.sub(imageROIPath + '/' + r'CLL-ROI-', imageName) # 保存图像的路径
