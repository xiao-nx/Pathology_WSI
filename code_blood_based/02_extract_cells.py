...
def read_xml(xml_path,patient_type='cll'):
    ...
    # Traverse the Second Level of the XML Document
    for child in root:
        ...
        for x in child.iter("name"):
            if x.text == patient_type:
                ...
                for sub_children in child.iter("bndbox"):
                    # ag Names and Attributes of Nodes in the Third Level
                    box = []
...
# Read image paths from a txt file and extract cell regions.
    ...
    with open(txt_path, "r") as f:
        ...
        for image_path in images_path:
            ...
            for i in range(len(bndboxes)):
                ...
                # CLL/Normal/Infected
                imgSaveName = save_path + patientID + '_' + str(i) + '.jpg'  # save images
                cv2.imencode('.jpg', cell_image)[1].tofile(imgSaveName)

    return

# Augment images in the list.
...
if __name__ == '__main__':
    # 数据的输入和输出路径
    data_path_dict = {
        "train_data": "../dataset/dataAnnotated/train.txt",
        "val_data": "../dataset/dataAnnotated/val.txt",
        "train_cells": "../dataset/dataBloodCells/trainset/",
        "val_cells": "../dataset/dataBloodCells/valset/",
    }
    # 批量读取文件夹数据
    ...
    #
    # # # # 保存训练数据和验证数据的路径
    save_txt_dir = '../dataSets/dataBloodCells/'
    ...
    # save_image_path = '../dataSets/dataBloodCells/normal_aug/'
    # if not os.path.exists(save_image_path):
    #     os.makedirs(save_image_path)
    # normal_images_list = []
    # cll_images_list = []
    # train_cells_txt = '../dataSets/dataBloodCells/train_raw.txt'
    ...
    # image_transform(normal_images_list, save_image_path)

    # 移动增广后的normal数据，建立trainset_aug
    # 重新写训练集的txt
    trainset_aug_path = '../dataSets/dataBloodCells/trainset_aug/'
    write_cells_path_to_txt(trainset_aug_path,save_txt_dir, 'train.txt')
