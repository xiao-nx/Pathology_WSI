...
data_transforms = {
    'train': transforms.Compose([
        ...
        transforms.RandomApply(image_transforms),
        transforms.RandomChoice(image_transforms), # 从image_transforms里选择一个执行
        transforms.RandomRotation(degrees=(-30,-120)),
...
# 数据路径
data_dir = '../dataSets/dataBloodCells/'
...
def train_model(model, criterion, optimizer, scheduler, modelName, num_epochs=25):
    ...
    best_acc = 0.0
    train_index_arrays = [["epoch", "loss", "acc", "acc_class0","acc_class1","auc", "precision", "recall", "F1_score"]]
    eval_index_arrays = [["epoch", "loss", "acc","acc_class0","acc_class1","auc", "precision", "recall", "F1_score"]]
    ...
    for epoch in range(num_epochs):
        ...
        writer.close()
    save_excel_path = './indicators/'
    if not os.path.exists(save_excel_path):
        os.makedirs(save_excel_path)
    write_to_excel(train_index_arrays, eval_index_arrays, save_excel_path + 'squeezenet1_1.xlsx')
...
def main():
    ...
    # for param in model.paramters(): # 冻结所有参数,不被更新
    #     param.requireds_grad = False
    # model.classifier[1] = nn.Conv2d(512,num_classes,kernel_size=(1,1),stride=(1,1))
    # model.numclasses = num_classes # 修改self.num_classes

    model_ft = models.detection.fasterrcnn_resnet50_fpn()
    model_ft = models.segmentation.deeplabv3_resnet50()
    ...
    model_ft = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler,
                           modelName='squeezenet1_1', num_epochs=35)
